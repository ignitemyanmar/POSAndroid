...
import com.ignite.pos.ReturnVoucherSlipActivity;
...
public class BtService extends PrintService implements PrinterClass {
	...
	public boolean write(byte[] bt) {
		...
		{
			/*Toast toast = Toast.makeText(context, context.getResources().getString(R.string.str_connecting), Toast.LENGTH_SHORT);
	        toast.show();*/
			return false;
		}
		mBTService.write(bt);
	...
	public void stopScan() {
		...
		if (VoucherSlipActivity.printerClass != null) {
			if (VoucherSlipActivity.printerClass.getState() == PrinterClass.STATE_SCANING) {
				mBTService.StopScan();
				mBTService.setState(PrinterClass.STATE_SCAN_STOP);
			}
		}
		
		if (ReturnVoucherSlipActivity.printerClass != null) {
			if (ReturnVoucherSlipActivity.printerClass.getState() == PrinterClass.STATE_SCANING) {
				mBTService.StopScan();
				mBTService.setState(PrinterClass.STATE_SCAN_STOP);
			}
		}
