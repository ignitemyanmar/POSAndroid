...
public class BlueToothService {
	...
	public void ConnectToDevice(String address) {
		...
		if (BluetoothAdapter.checkBluetoothAddress(address)) {
			...
			Log.i("",address+ " device is not valid.");
		}
	}
	...
	private class ConnectThread extends Thread {
		...
		public ConnectThread(BluetoothDevice device) {
			...
			try {
				...
				//tmp = device.createRfcommSocketToServiceRecord(MY_UUID);   // Old code 
				
				if (Build.VERSION.SDK_INT < 9) { 			//edit by su wai 23 mar 2015
					try {
					...
					Method m = null; //edit by su wai 23 mar 2015
					
					try {
						m = device.getClass().getMethod("createInsecureRfcommSocket", new Class[] {int.class});  //edit by su wai 23 mar 2015
						try {
							tmp = (BluetoothSocket) m.invoke(device, 1);    //edit by su wai 23 mar 2015
		...
		public void run() {
			...
			Log.i(TAG, "Hi, BEGIN mConnectThread");
		...
		public void run() {
			...
			while (true) {
				try {
					...
					if (mmInStream.available() > 0) {		//Edited by Su Wai 24/3/2015
                         // Read from the InputStream
                         bytes = mmInStream.read(buffer);
					}
