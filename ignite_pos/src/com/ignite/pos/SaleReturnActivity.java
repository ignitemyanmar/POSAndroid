	...
	private void saveReturnVoucher()
	{
		...
		if (lv_return_voucher.getAdapter() != null && lv_return_voucher.getAdapter().getCount() > 0) {
			...
			for (int i = 0; i < saleReturnList.size(); i++) {
				...
				selectLedgerList = ledgerControl.select(sr.getItemid(), sr.getReturnDate());
				
				Log.i("", "All in Ledger : "+ledgerControl.select(sr.getItemid(), sr.getReturnDate()).toString());
				...
				if (selectLedgerList.size() > 0) {
							...
							if (sr.getItemid().equals(ledger.getItemId())) {
								...
								Log.i("", "Return Total qty: "+totalReturnQty);
								
								updateLedgerList.add(new Ledger(sr.getItemid(), sr.getItemName(), sr.getReturnDate()
				...
				if (itemObj.getQty().equals("")) {
					oldStkQty = 0;
					newStkQty = oldStkQty + Integer.valueOf(sr.getReturnQty());
				}else {
					oldStkQty = Integer.valueOf(itemObj.getQty());
					newStkQty = oldStkQty + Integer.valueOf(sr.getReturnQty());
				}
				
				ledgerList.add(new Ledger(sr.getItemid(), sr.getItemName(), sr.getReturnDate()
						, oldStkQty, 0, 0, newStkQty, sr.getReturnQty()));
