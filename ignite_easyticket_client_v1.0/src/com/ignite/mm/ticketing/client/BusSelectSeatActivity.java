...
@SuppressLint("SimpleDateFormat") public class BusSelectSeatActivity extends BaseSherlockActivity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		//Get Date format to check late date
		try {
			tripDate = sdf.parse(Date);
		...
		//Get Time format to check late time
		try {
			//Get only 06:00 AM format
			String time = Time.substring(0, 8);
			
			SimpleDateFormat formatShort = new SimpleDateFormat("hh:mm aa");
			tripTime = formatShort.parse(time);
		} catch (ParseException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
		try {
			tTime = sdf2.parse(todayTime);
		} catch (ParseException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		...
		txt_dept_date.setText("ထြက္ခြာမည့္ ေန႔ရက္ : "+ Date);
...
/*						if(!AgentID.equals("0")){
					...
					if(jsonObject.getString("status").equals("1")){
						...
						if(jsonObject.getBoolean("can_buy") && jsonObject.getString("device_id").equals(DeviceUtil.getInstance(BusSelectSeatActivity.this).getID())){
							...
							if(isBooking == 0){
		        				...
		        				finish();
		        				Log.i("", "Booking ...........................................");
		        				SKToastMessage.showMessage(BusSelectSeatActivity.this, "Booking မွာ�?�ပီး�?�ပီ ျဖစ္ပါသည္ !", SKToastMessage.SUCCESS);
		        			...
		        			dialog.dismiss();
		        			SKToastMessage.showMessage(BusSelectSeatActivity.this, "သင္ မွာယူေသာ လက္ မွ�?္ မ်ားမွာ စက�?�န့္ပိုင္ အ�?ြင္း �?စ္ ျ�?ားသူ ယူ သြားေသာေ�?�ကာင့္ သင္မွာေသာလက္မွ�?္မ်ား မရ�?ိုင္ေ�?ာ့ပါ�?� ေက်းဇူးျပဳ�?�ပီး �?စ္ျ�?ားလက္ မွ�?္ မ်ား ျပန္ေရႊးေပးပါ�?��?�", SKToastMessage.ERROR);
		        			getSeatPlan();
		        		}
	...
	private OnItemClickListener itemClickListener = new OnItemClickListener() {
		...
		public void onItemClick(AdapterView<?> parent, View view, int position,
			...
			txt_price.setText("ေစ်း�?ႈန္း :"+ BusSeats.get(0).getSeat_plan().get(position).getPrice()+" Ks");
			...
			if(v == btn_now_booking){
				if(SelectedSeat.length() != 0){
					...
					if(connectionDetector.isConnectingToInternet()){
						...
						isBooking = 1;
						AgentID = "1";
						CustName = AppLoginUser.getUserName();
						CustPhone = "";
						RemarkType = 0;
						Remark = "";
						
						Log.i("", "Agent ID: "+AgentID);
